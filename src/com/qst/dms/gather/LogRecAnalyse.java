...
public class LogRecAnalyse extends DataFilter implements IDataAnalyse {
	...
	public void doFilter() {
		...
		for(LogRec l:logs){
			if(l.getLogType()==LogRec.LOG_IN)logIns.add(l);// 添加到“登录”日志集合中
			else if(l.getLogType()==LogRec.LOG_OUT)logOuts.add(l);// 添加到“登出”日志集合中
		}
	...
	public ArrayList<MatchedLogRec> matchData() {
		...
		try {
			if(matchLogs.size()==0){
				throw new DataAnalyseException("沒有匹配的日志数据！");
			}
